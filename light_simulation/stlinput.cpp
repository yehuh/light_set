...
GLint sum_of_plan(char fname[20])  //預先讀取,看文件有幾行,平面有幾個
{
	     string text;
	     string text1;
...
{	
	set_point_and_vector *points ;
	points = new set_point_and_vector[num];
	
	for(int i = 0 ; i < num ; i++)
	{
		points[i].r.X = (surfaces[i].obj_plan.plan[0].X+surfaces[i].obj_plan.plan[1].X+surfaces[i].obj_plan.plan[2].X)/3;
		points[i].r.Y = (surfaces[i].obj_plan.plan[0].Y+surfaces[i].obj_plan.plan[1].Y+surfaces[i].obj_plan.plan[2].Y)/3;
        points[i].r.Z = (surfaces[i].obj_plan.plan[0].Z+surfaces[i].obj_plan.plan[1].Z+surfaces[i].obj_plan.plan[2].Z)/3;
        points[i].nvector = surfaces[i].nomalvector;
	}
	return points;
}
